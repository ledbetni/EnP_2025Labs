...
#Interpretation:
Since the first histogram is cut off at the 99th percentile, suggesting that most bid prices are concentrated at relatively low price values, and suggests that we may have right-skewness in the data. In the second histogram, we see that we do in fact have a strongly right-skewed distribution. In this second histogram, we do not truncate the distribution at the 99th percentile, and thus we see a large tail of extreme outliers. The data dictionary dictates that we should remove many of these outliers, as these are "invalid." Therefore, to obtain a more accurate view of typical bidding behavior, these extreme outliers should be removed.
