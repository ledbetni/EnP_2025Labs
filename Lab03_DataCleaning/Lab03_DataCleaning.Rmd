...
>    - Do you see obvious anomalies or strange values?
        ...
        `PRICE` is a character value in the raw dataset.
        There are no obvious strange values, though the dataset is large. I selected          a random sampling of 20 `PRICE` values, and they are all valid. The type should not be character, however. 
...
*HENRY FINISH*
Negative values, sentinel codes, and impossibly high values will be set to NA.
...
*HENRY FINISH*
Valid Oregon zip codes are 5 digits and have pattern "97xxx". All others are NA.
...
*HENRY FINISH (Regular expressions like ^[^0-9.-] allow parsing of specific characters in specific patterns* 
...
*HENRY FINISH (entires that failed to parse didnt abide the time format standards above)*
...
*HENRY FINISH (full row duplicates dropped, leaving 1 copy. Same auction ID with different values kept)*
...
*HENRY FINISH (removed values outside oregon boundaries)*
