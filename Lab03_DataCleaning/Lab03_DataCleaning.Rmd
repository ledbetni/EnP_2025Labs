...
The most time-consuming work was writing regular expressions to turn messy strings into valid values, and it was surprising how much hidden bad data they revealed. Our most useful tools were the dplyr and stringr workflows—especially mutate(), case_when(), str_*(), parse_date_time(), and distinct()/count(). We did use AI to help debug errors, but next time we’ll first try to understand and fix issues ourselves and only lean on AI as a backup.
